...
export default function OutputPanel({ output, activeTab, processes }) {
  ...
  // Aktif sekmede gösterilecek çıktı - gerçek çıktı veya örnek
  const displayOutput = processOutput || (processId ? getSampleOutput() : output);
  
  // Panel içeriğini oluşturma
  const renderContent = () => {
    if (!displayOutput) {
      return (
        <div className="h-full flex items-center justify-center">
          <div className="text-center space-y-3">
            ...
            <p className="text-gray-500">
              {processId 
                ? `Run ${processName} to see actual output here` 
                : 'Run a process to see the output here'}
            </p>
          </div>
        </div>
      );
        ...
        <section>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="prose prose-sm max-w-none text-gray-600">
              <ReactMarkdown>{displayOutput.content}</ReactMarkdown>
            </div>
          </div>
        </section>
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header - Süreç adını gösteren başlık */}
